...
class ImapHook(BaseHook):
    ...
    def list_mail_ids_desc(
        self, mail_folder: str = "INBOX", mail_filter: str = "All"
    ) -> Iterable[str]:
        self.mail_client.select(mail_folder)
        return self._list_mail_ids_desc(mail_filter)

    def close(self):
        self.mail_client.close()

    def fetch_mail_body(self, mail_id: str) -> (str, str):
        if not self.mail_client:
        ...
        return mail_body_str, message["Message-ID"].strip()
    ...
    def _retrieve_mails_attachments_by_name(
        ...
        for mail_id in self._list_mail_ids_desc(mail_filter):
            response_mail_body = self.fetch_mail_body(mail_id)
    ...
    def _check_mail_body(
        ...
        if mail.has_attachments():
            return mail.get_attachments_by_name(
                name, check_regex, find_first=latest_only
            )
        return []
...
class AirflowIMAPPlugin(AirflowPlugin):
    name = "imap_plugin"
    hooks = [ImapHook]
